...
const HomePage = () => {
  ...
  return (
    <Box sx={{ mt: 2 }}>
      ...
      >
        Welcome Back, {userName.split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join(' ')}
      </Typography>
      {selectedFunction == ''?
        ...
        <Box>
          ...
          <List>
            {functions.map((func, index) => (
              <ListItem button key={index} onClick={() => setSelectedFunction(func.name)} 
              sx={{ 
      ...
      <Grid container spacing={2} justifyContent="center">
        ...
        <Grid item xs={12} sm={6}>
          <Box sx={{ 
            ...
            {selectedDataset == ""?
              <UploadedDatasets  userName={userName} setSelectedDataset={setSelectedDataset}/>
              :
              <SimilarFarmers userName={userName} selectedDataset={selectedDataset}/>
            }
          </Box>
        </Grid>
      </Grid>
