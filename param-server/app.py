...
def send_request_to_train_local_model(
    token, user_id, metadata, updates_list, hyperparameters, lock
):
    try:
        headers = {
...
def aggregate_updates(client_updates):
    ...
    aggregated_weights = [
        np.mean([updates["weights"][i] for updates in client_updates], axis=0)
        for i in range(len(client_updates[0]["weights"]))
    ]
...
def evaluate_model(model, test_data, test_labels):
    ...
    test_data_reshaped = (
        np.expand_dims(test_data, axis=-1)
        if model.layers[0].input_shape[-1] == 1 and len(test_data.shape) == 2
        else test_data
    )
...
def start_training_process(collaborators, metadata, model_id, hyperparameters, token):
    try:
        ...
        lock = (
            threading.Lock()
        )  #  Use a lock for thread-safe access to the updates list.
        ...
        for collaborator_id in collaborators:
            thread = threading.Thread(
                target=send_request_to_train_local_model,
                args=(
                    token,
                    collaborator_id,
                    metadata,
        ...
        new_values = {"$set": {"modelWeights": string_model_weights, "status": "Done"}}
        if model_info:
...
def train():
    try:
        ...
        thread = threading.Thread(
            target=start_training_process,
            args=(
                data_received.get("collaborators"),
                data_received.get("metadata"),
                data_received.get("model_id"),
                data_received.get("hyperparameters"),
                token,
            ),
        )
        thread.start()
...
def test():
    ...
    messages = db["messages"].find(
        {"$or": [{"senderID": "osamazafar98"}, {"receiverID": "osamazafar98"}]}
    )
    ...
    for message in messages:
        partner_id = (
            message["receiverID"]
            if message["senderID"] == "osamazafar98"
